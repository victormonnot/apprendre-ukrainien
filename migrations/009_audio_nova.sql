CREATE TABLE audio_clips_next (
  id TEXT PRIMARY KEY NOT NULL,
  user_id TEXT NOT NULL REFERENCES profiles (id),
  cache_key TEXT NOT NULL CHECK (length(cache_key) = 64),
  text TEXT NOT NULL CHECK (length(text) BETWEEN 1 AND 1000),
  voice_id TEXT NOT NULL CHECK (voice_id IN ('macos-lesya', 'openai-marin', 'openai-cedar', 'openai-nova')),
  ...
  CHECK ((provider = 'macos' AND voice_id = 'macos-lesya') OR
    (provider = 'openai' AND voice_id IN ('openai-marin', 'openai-cedar', 'openai-nova')))
) STRICT;

INSERT INTO audio_clips_next (
...
ALTER TABLE audio_clips_next RENAME TO audio_clips;
